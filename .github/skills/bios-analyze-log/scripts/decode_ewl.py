...
# Supported platforms and their database locations (relative to skill root)
SUPPORTED_PLATFORMS = ("egs", "bhs", "oks")
_SKILL_ROOT = Path(__file__).resolve().parent.parent  # bios-analyze-log/
_REFERENCES_DIR = _SKILL_ROOT / "references"


def _load_json(path, default=None):
    """Load a JSON file and return parsed data, or *default* on failure."""
    try:
...
def _resolve_platform_db(platform, filename):
    """Return the path to *filename* inside references/<platform>/."""
    return _REFERENCES_DIR / platform.lower() / filename


class EWLDecoder:
    """Decoder for Enhanced Warning Log codes"""

    def __init__(self, platform="oks", db_path=None, rc_db_path=None):
        """Initialize decoder with platform-specific code databases.

        Args:
            platform: one of 'egs', 'bhs', 'oks' (default 'oks').
            db_path: explicit override for EWL database path.
            rc_db_path: explicit override for RC Fatal database path.
        """
        ...
        if db_path is None:
            db_path = _resolve_platform_db(self.platform, "ewl_codes_database.json")
        else:
            db_path = Path(db_path)

        if rc_db_path is None:
            rc_db_path = _resolve_platform_db(self.platform, "rc_fatal_errors_database.json")
        else:
        ...
        # IPSD is OKS-specific
        self.IPSD_ERRORS = {}
        if self.platform == "oks":
            ipsd_path = _resolve_platform_db("oks", "ipsd_codes_database.json")
            ipsd_raw = _load_json(ipsd_path, default={})
            if ipsd_raw:
                self.IPSD_ERRORS = {
                    int(k, 16): v["description"] for k, v in ipsd_raw.items()
                }

        if self.db:
            print(f"✓ [{self.platform.upper()}] Loaded {len(self.db)} EWL codes", file=sys.stderr)
        if self.rc_db:
            total_rc_minors = sum(len(info.get('minors', {})) for info in self.rc_db.values())
            print(f"✓ [{self.platform.upper()}] Loaded {len(self.rc_db)} RC fatal major codes ({total_rc_minors} minors)", file=sys.stderr)
        if self.IPSD_ERRORS:
            print(f"✓ [{self.platform.upper()}] Loaded {len(self.IPSD_ERRORS)} IPSD codes", file=sys.stderr)

    def load_database(self, path):
        """Load JSON database file."""
        return _load_json(path, default=None)
    ...
    def parse_log(self, log_text):
        ...
        # Pattern 3: IPSD errors "ERROR: C8XXXXXXX:..." (OKS only)
        pattern3 = r'ERROR:\s*(C8[0-9A-Fa-f]{7}):([^\s]+)' if self.platform == 'oks' else None
        ...
        while i < len(lines):
            ...
            # Pattern 3: IPSD (OKS-only — skip on other platforms)
            if self.platform == "oks":
    ...
    def generate_summary(self, codes):
        ...
        # Process IPSD codes
        if self.platform == 'oks' and ipsd_codes:
...
def main():
    ...
    parser.add_argument('--platform', choices=SUPPORTED_PLATFORMS, default='oks',
                        help='Target platform: egs, bhs, or oks (default: oks)')
    ...
    parser.add_argument('--db', default=None, help='Path to EWL database (overrides platform default)')
    parser.add_argument('--json', action='store_true', dest='json_output',
                        help='Emit machine-readable JSON instead of markdown')

    args = parser.parse_args()

    decoder = EWLDecoder(platform=args.platform, db_path=args.db)
