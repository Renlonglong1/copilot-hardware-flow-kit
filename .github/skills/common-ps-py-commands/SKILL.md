...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
```
...
Get-Content .\config\local\hardware-flow.json -Raw | ConvertFrom-Json | Out-Null
```
...
ssh -o BatchMode=yes -o ConnectTimeout=10 -o UserKnownHostsFile=$kh -o StrictHostKeyChecking=accept-new debug@lab-control-05 "powershell.exe -NoProfile -NonInteractive -EncodedCommand $enc"
```

Known mapping:

```text
lab-control-05 -> 192.0.2.53, BHS/GNRSP BKC, COM3/COM4
lab-control-12 -> 192.0.2.44, OKS/Johnson City BKC
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-BhsUplr2ValidatedFlow.ps1 -ConfigPath .\config\local\hardware-flow.json -CloseGuiConflicts -RunMlc
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-PowerSplitter.ps1 -Action poweroff -ConfigPath .\config\local\hardware-flow.json
powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-PowerSplitter.ps1 -Action poweron -ConfigPath .\config\local\hardware-flow.json
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-Emulator.ps1 -Action program -Chip MX66U1G45G -BinFile '<REMOTE_BIN_PATH>' -ConfigPath .\config\local\hardware-flow.json
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
powershell -ExecutionPolicy Bypass -File .\scripts\Invoke-BhsUplr2ValidatedFlow.ps1 -ConfigPath .\config\local\hardware-flow.json -CloseGuiConflicts
```

Remote helper direct use only if needed:

```powershell
scp .\scripts\Capture-SerialPort.ps1 debug@lab-control-05:C:/Users/debug/Desktop/flow_logs/Capture-SerialPort.ps1
scp .\scripts\Invoke-RemoteBootCapture.ps1 debug@lab-control-05:C:/Users/debug/Desktop/flow_logs/Invoke-RemoteBootCapture.ps1
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\YOUR_USER\copilot-hardware-flow-kit'
scp .\scripts\Invoke-RemoteMlcSerial.ps1 debug@lab-control-05:C:/Users/debug/Desktop/flow_logs/Invoke-RemoteMlcSerial.ps1
```
