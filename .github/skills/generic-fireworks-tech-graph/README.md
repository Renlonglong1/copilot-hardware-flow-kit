...
```bash
npx skills add yizhiyanhua-ai/generic-fireworks-tech-graph
```

This skill is installed from the GitHub repository. The npm package page is the public package/distribution page:

```text
https://www.npmjs.com/package/@yizhiyanhua-ai/generic-fireworks-tech-graph
```
...
```bash
npx skills add yizhiyanhua-ai/generic-fireworks-tech-graph --force -g -y
```
...
```bash
git clone https://github.com/yizhiyanhua-ai/generic-fireworks-tech-graph.git ~/.claude/skills/generic-fireworks-tech-graph
```
...
| | Mermaid | draw.io | **generic-fireworks-tech-graph** |
|--|---------|---------|--------------------------|
...
Mermaid is great for quick inline diagrams in markdown. draw.io is great for manual polishing. `generic-fireworks-tech-graph` is optimized for **describing a system and getting a polished diagram immediately**, without writing DSL syntax or clicking around a GUI.
...
MIT © 2025 generic-fireworks-tech-graph contributors
