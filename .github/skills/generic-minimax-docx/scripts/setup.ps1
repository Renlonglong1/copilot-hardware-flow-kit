# generic-minimax-docx Environment Setup & Initialization Script (Windows PowerShell)
...
Write-Host "============================================"
Write-Host "  generic-minimax-docx Setup & Initialization (Windows)"
...
# --- Build Project ---
Step "Building generic-minimax-docx .NET project"
...
if (-not $SkipVerify) {
    ...
    $testOutput = Join-Path $env:TEMP "generic-minimax-docx-setup-test-$PID.docx"
    ...
    if ($testExitCode -eq 0 -and (Test-Path $testOutput)) {
        ...
        Log "Test passed - generic-minimax-docx is ready to use!"
    } else {
