...
| `diagram` | Technical diagram generated by generic-fireworks-tech-graph skill | `description`, `diagram_type`?, `path`?, `caption`?, `figure`? |
...
## Diagram Integration (generic-fireworks-tech-graph)

When the PDF content requires technical diagrams (architecture, data flow, flowchart, sequence, state machine, etc.), use the **generic-fireworks-tech-graph** skill to generate production-quality SVG+PNG diagrams before PDF rendering.

**Workflow:**

1. Identify diagram needs in the content — any `diagram` block type or complex visuals that exceed simple matplotlib charts
2. Run the **Fireworks Tech Graph** agent (subagent) with a description of the diagram to generate
3. The agent produces an SVG file and exports a PNG via `rsvg-convert`
4. Reference the generated PNG path in `content.json` as a `figure` or `diagram` block

**When to use generic-fireworks-tech-graph vs built-in matplotlib:**

| Use case | Renderer |
|---|---|
| Simple bar/line/pie chart with numeric data | Built-in `chart` block (matplotlib) |
| Simple process flow (≤6 nodes, linear) | Built-in `flowchart` block (matplotlib) |
| Architecture diagram, multi-layer system | **generic-fireworks-tech-graph** |
| Data flow with labeled arrows and legend | **generic-fireworks-tech-graph** |
| Sequence diagram (lifelines, messages) | **generic-fireworks-tech-graph** |
| UML class/state/ER diagrams | **generic-fireworks-tech-graph** |
| Complex flowchart (>6 nodes, branches, loops) | **generic-fireworks-tech-graph** |
| Network topology | **generic-fireworks-tech-graph** |
| Agent/memory architecture | **generic-fireworks-tech-graph** |

**`diagram` block schema:**
```json
{"type": "diagram",
 "description": "3-layer architecture: Client → API Gateway → Microservices → Database",
 "diagram_type": "architecture",
 "path": "/tmp/diagrams/arch_diagram.png",
 "caption": "System Architecture Overview",
 "figure": true}
```

**Pre-render step** (execute before `render_body.py`):

```bash
# Use Fireworks Tech Graph agent to generate the diagram
# Agent produces: /tmp/diagrams/arch_diagram.svg + .png

# Or use the helper script directly:
python3 ~/.copilot/skills/generic-fireworks-tech-graph/scripts/generate-from-template.py \
  architecture /tmp/diagrams/arch_diagram.svg '{"title":"System Architecture","nodes":[...],"arrows":[...]}'

# Validate and export PNG:
rsvg-convert -w 1920 /tmp/diagrams/arch_diagram.svg -o /tmp/diagrams/arch_diagram.png
```

**Rule:** If a `diagram` block has no `path` field (PNG not yet generated), the agent MUST first invoke the Fireworks Tech Graph agent with the `description` and `diagram_type` to generate the diagram, then update the `path` before rendering the PDF.
