#!/usr/bin/env bash
# make.sh — generic-minimax-pdf unified CLI
...
[
  ...
  {"type":"body",    "text":"This document was generated by generic-minimax-pdf — a skill for creating visually polished PDFs. Every design decision is rooted in the document type and content, not a generic template."},
...
JSON
  ...
  cmd_run \
    --title   "generic-minimax-pdf demo" \
    --type    "report" \
    --author  "generic-minimax-pdf skill" \
    --date    "$(date '+%B %Y')" \
...
main() {
  if [[ $# -lt 1 ]]; then
    bold "generic-minimax-pdf — make.sh"
    echo ""
