...
# Common CJK TTF font search paths (order = preference)
_CJK_FONT_CANDIDATES = [
    ...
    os.path.expanduser("~/.local/share/fonts/NotoSansSC-Regular.ttf"),
]


def _find_cjk_font() -> str | None:
    """Return the first available CJK TTF font path, or None."""
...
def _has_cjk(content: list) -> bool:
    """Return True if any text block contains CJK characters."""
    for item in content:
        for key in ("text", "caption"):
            text = item.get(key, "")
            if text and any("\u4e00" <= ch <= "\u9fff" or
                           "\u3400" <= ch <= "\u4dbf" or
                           "\uf900" <= ch <= "\ufaff" for ch in text):
                return True
        # Check table headers/rows
        for h in item.get("headers", []):
            if any("\u4e00" <= ch <= "\u9fff" for ch in str(h)):
                return True
...
def _register_cjk_fonts(font_path: str):
    """Register CJK font as 'CJK' and 'CJK-Bold' for ReportLab."""
    pdfmetrics.registerFont(TTFont("CJK", font_path))
    pdfmetrics.registerFont(TTFont("CJK-Bold", font_path))
    pdfmetrics.registerFontFamily(
        "CJK", normal="CJK", bold="CJK-Bold",
...
_RESETS_NUMBERED = frozenset({
    ...
    "image", "figure", "diagram", "code", "math", "chart", "flowchart",
...
def build_story(content: list, tokens: dict, styles: dict) -> list:
    ...
    for item in content:
        ...
        elif kind == "diagram":      _add_figure(story, item, ctx)
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    ...
    # Auto-detect CJK content and switch to CJK-capable font
    if _has_cjk(content):
        cjk_path = _find_cjk_font()
        if cjk_path:
            _register_cjk_fonts(cjk_path)
