...
4. `config\hardware-flow.dbgsh05.json`
5. `config\bkc-remote-inventory.dbgsh05.json` if the request involves BKC image selection
6. `docs\bkc_remote_inventory_dbgsh05.md` if a human-readable BKC inventory is useful
...
If the request lacks a server, prefer the validated BHS control server in the config. If the request mentions `10.239.84.44` but the target is BHS/GNRSP, verify hostname/IP first because historical DNS/SSH evidence showed:

```text
dbgsh05.ccr.corp.intel.com -> 10.239.84.53, BHS/GNRSP BKC folder, COM3/COM4
dbgsh12.ccr.corp.intel.com -> 10.239.84.44, OKS/Johnson City BKC folder
```

Use a temporary `UserKnownHostsFile` if host key aliases conflict.
...
- For BHS uPLR/DBGSH05, 64 MB IFWI images can be valid and were used successfully in prior validation.
- `_2s` means 2S/2P.
...
```text
control server: dbgsh05.ccr.corp.intel.com / 10.239.84.53
BKC folder: C:\Users\debug\Desktop\BKC\GNRSP_25WW03
bin: C:\Users\debug\Desktop\BKC\GNRSP_25WW03\BHSDCRB1.IPC.3544.P29.2412180233_GB81000380_GA80000680_SC83000330_RB8A0000C0_SP_IP_Clean_Debug_1.bin
